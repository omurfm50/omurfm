...
function ChattersNetRoom({ userCount, isLoading, error, refreshUserCount, onOpenRules }) {
  ...
  return (
    <section aria-labelledby="chattersnet-room-title" className="overflow-hidden border border-rose-300/15 bg-[#0e080a] shadow-2xl shadow-black/30">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.08] bg-[#160a0f] px-4 py-3.5 sm:px-5">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={onOpenRules} className="inline-flex items-center gap-2 border border-white/10 bg-white/[0.04] px-3 py-2 text-sm font-medium text-stone-200 transition hover:bg-white/[0.08] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-300"><ShieldCheck size={16} aria-hidden="true" /> Kurallar</button>
          <button type="button" onClick={reloadChat} className="inline-flex items-center gap-2 bg-rose-700 px-3 py-2 text-sm font-semibold text-white transition hover:bg-rose-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-300"><RefreshCw size={16} aria-hidden="true" /> Sohbeti Yenile</button>
        </div>
      </div>
